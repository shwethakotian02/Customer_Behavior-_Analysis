...
--Q2.which customer used discount but spent still more than average purchase amount?
select customer_id, purchase_amount from customer
where discount_applied='Yes' and purchase_amount>(select avg(purchase_Amount)from customer);

--Q3 what are the top5 products with highest average review rating?
select item_purchased, ROUND(avg(review_rating)::numeric,2) as "avg_rating" from
customer
...
select subscription_status, ROUND(avg(purchase_amount),2) as "avg_pur", sum(purchase_amount) as "total_rev", 
COUNT(customer_id) as total_Cus
from customer
group by subscription_status
order by avg_pur,total_rev desc;
...
select customer_segment,count(*) as total_number_purchases
from customer_type
